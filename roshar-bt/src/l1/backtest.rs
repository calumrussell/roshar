...
mod tests {
    ...
    #[test]
    fn test_multi_symbol_performance_metrics() {
        // Three price updates per symbol so we get meaningful returns
        let aave0 = r#"1000000000000000000 {"channel":"candle","data":{"T":100,"c":"100.0","h":"100.0","i":"1m","l":"100.0","n":1,"o":"100.0","s":"AAVE","t":100,"v":"0.21"}}"#.to_string();
        let btc0 =  r#"1005000000000000000 {"channel":"candle","data":{"T":200,"c":"50000.0","h":"50000.0","i":"1m","l":"50000.0","n":1,"o":"50000.0","s":"BTC","t":200,"v":"1.0"}}"#.to_string();
        let aave1 = r#"1010000000000000000 {"channel":"candle","data":{"T":101,"c":"104.0","h":"104.0","i":"1m","l":"104.0","n":1,"o":"104.0","s":"AAVE","t":101,"v":"0.21"}}"#.to_string();
        let btc1 =  r#"1015000000000000000 {"channel":"candle","data":{"T":201,"c":"51000.0","h":"51000.0","i":"1m","l":"51000.0","n":1,"o":"51000.0","s":"BTC","t":201,"v":"1.0"}}"#.to_string();
        let aave2 = r#"1020000000000000000 {"channel":"candle","data":{"T":102,"c":"108.0","h":"108.0","i":"1m","l":"108.0","n":1,"o":"108.0","s":"AAVE","t":102,"v":"0.21"}}"#.to_string();
        let btc2 =  r#"1025000000000000000 {"channel":"candle","data":{"T":202,"c":"49000.0","h":"49000.0","i":"1m","l":"49000.0","n":1,"o":"49000.0","s":"BTC","t":202,"v":"1.0"}}"#.to_string();

        use crate::source::MultiplexedFeed;
        use rust_decimal::Decimal;

        let src_aave = EventVecSource::new(vec![aave0, aave1, aave2]);
        let src_btc = EventVecSource::new(vec![btc0, btc1, btc2]);
        ...
        // Step through first two events to establish initial prices
        let _ = bt.step(); // AAVE @ 100
        let _ = bt.step(); // BTC @ 50000

        // Open positions: long AAVE, short BTC
        let _ = bt.execute_market_order(
            "AAVE",
            OrderRequest::new(Side::Buy, 10.0, None, crate::types::OrderType::Market),
        );
        let _ = bt.execute_market_order(
            "BTC",
            OrderRequest::new(Side::Sell, 1.0, None, crate::types::OrderType::Market),
        );

        // Step through remaining events to generate returns
        // AAVE: 100 -> 104 -> 108 (up)
        // BTC: 50000 -> 51000 -> 49000 (up then down)
        let _ = bt.step(); // AAVE @ 104
        let _ = bt.step(); // BTC @ 51000
        let _ = bt.step(); // AAVE @ 108
        let _ = bt.step(); // BTC @ 49000

        let perf = bt.get_performance_metrics();

        // Should have recorded returns
        assert!(
            !perf.get_returns_history().is_empty(),
            "Should have return history after multi-symbol steps"
        );

        // Cumulative return should be non-zero since prices moved with open positions
        assert_ne!(
            perf.get_cumulative_return(),
            Decimal::ZERO,
            "Cumulative return should be non-zero with positions and price movement"
        );

        // Price history should be populated
        assert!(
            perf.get_prices().len() >= 4,
