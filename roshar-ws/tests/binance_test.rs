...
async fn test_binance_websocket() {
    ...
    client.send_frame(&subscribe_frame)
        .await
        ...
        match timeout(receive_timeout, reader.read_frame()).await {
            Ok(Ok(frame)) => {
                ...
                if frame_count % 100 == 0 || frame_count <= 10 {
                    println!("Frame #{}: opcode={:?}, fin={}, payload_len={}, time={:?}", 
                             frame_count, frame.opcode, frame.fin, frame.payload.len(), frame_duration);
                }
            }
            Ok(Err(e)) => {
    ...
    println!("Frames per second: {:.2}", frame_count as f64 / total_time.as_secs_f64());
    println!("Processing times (<1ms): {} frames, avg: {:?}", processing_times.len(), avg_processing);
    println!("Wait times (>10ms): {} frames, avg: {:?}", wait_times.len(), avg_wait);
}
...
async fn test_binance_websocket_tungstenite() {
    ...
    ws_stream.send(subscribe_message)
        .await
    ...
    while frame_count < target_frames {
        ...
        match timeout(receive_timeout, read.next()).await {
            Ok(Some(Ok(message))) => {
                ...
                if frame_count % 100 == 0 || frame_count <= 10 {
                    println!("Frame #{}: message={:?}, payload_len={}, time={:?}", 
...
async fn test_synthetic_performance_urithiru() {
    ...
    println!("Reading {} frames with urithiru (synthetic)...", target_frames);
    ...
    println!("Frames per second: {:.2}", target_frames as f64 / total_time.as_secs_f64());
    println!("Throughput: {:.2} MB/s", (frame_data.len() as f64 / 1024.0 / 1024.0) / total_time.as_secs_f64());
}

#[tokio::test]
async fn test_synthetic_performance_tungstenite() {
    use tokio_tungstenite::{WebSocketStream, tungstenite::protocol::Role};
    use tokio_tungstenite::tungstenite::protocol::frame::coding::{OpCode, Data};
    ...
    println!("Generating {} synthetic frames for tokio_tungstenite...", target_frames);
    for i in 0..target_frames {
        let payload = &payloads[i % payloads.len()];
        // Create a server-to-client frame (unmasked)
    ...
    println!("Reading {} frames with tokio_tungstenite (synthetic)...", target_frames);
    ...
    println!("P50 time per frame: {:?}", p50);
    println!("P99 time per frame: {:?}", p99);
    println!("Frames per second: {:.2}", timings.len() as f64 / total_time.as_secs_f64());
    println!("Throughput: {:.2} MB/s", (frame_data_len as f64 / 1024.0 / 1024.0) / total_time.as_secs_f64());
}
