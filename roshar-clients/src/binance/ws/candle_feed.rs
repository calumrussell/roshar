use roshar_types::{BinanceCandleMessage, Candle};
...
impl CandleFeedHandle {
    /// Add a symbol subscription dynamically
    pub async fn add_subscription(&self, symbol: &str) -> Result<(), String> {
        self.command_tx
    ...
    /// Remove a symbol subscription dynamically
    pub async fn remove_subscription(&self, symbol: &str) -> Result<(), String> {
        self.command_tx
    ...
    pub async fn get_latest_candle(&self, symbol: &str) -> Result<Option<Candle>, String> {
...
/// Manages Candle feed for Binance
pub(crate) struct CandleFeed {
    ws_manager: Arc<Manager>,
    conn_name: String,

    // Candle data storage: symbol -> latest candle
...
impl CandleFeed {
    pub fn new(ws_manager: Arc<Manager>) -> Self {
        ...
        Self {
            ws_manager,
            conn_name: "binance-candle".to_string(),
    ...
    pub async fn run(mut self) {
        ...
        log::info!(
            "WebSocket reader set up for Binance candle feed: {}",
            self.conn_name
        );
        ...
        if let Err(e) = self.ws_manager.new_conn(&self.conn_name, ws_config) {
            log::error!(
                "Failed to create Binance candle WebSocket connection: {}",
                e
            );
            return;
        }
                ...
                msg = recv.recv() => {
                    match msg {
                        Ok(roshar_ws_mgr::Message::SuccessfulHandshake(_name)) => {
                            log::info!("Binance candle feed WebSocket connected: {}", self.conn_name);
                        ...
                        Ok(roshar_ws_mgr::Message::ReadError(_name, err)) => {
                            log::error!("Websocket read error in Binance candle feed: {}", err);
                            self.is_connected = false;
                        }
                        Ok(roshar_ws_mgr::Message::WriteError(_name, err)) => {
                            log::error!("Websocket write error in Binance candle feed: {}", err);
                        }
                        Ok(roshar_ws_mgr::Message::PongReceiveTimeoutError(_name)) => {
                            log::warn!("Pong receive timeout in Binance candle feed");
                            self.is_connected = false;
                        }
                        Ok(_) => {}
                        Err(e) => {
    ...
    fn resubscribe_all(&self) {
        for symbol in &self.subscriptions {
            ...
            if let Err(e) = self.ws_manager.write(
                ...
                log::error!("Failed to resubscribe to candle for {}: {}", symbol, e);
            }
        }
    }
            ...
            CandleCommand::Add { symbol } => {
                if self.subscriptions.insert(symbol.clone()) {
                    ...
                    if let Err(e) = self.ws_manager.write(
                        ...
                        log::error!("Failed to subscribe to candle for {}: {}", symbol, e);
                        self.subscriptions.remove(&symbol);
                    } else {
                        log::info!("Subscribed to Binance candle feed for {}", symbol);
                    }
                }
            }
            CandleCommand::Remove { symbol } => {
                if self.subscriptions.remove(&symbol) {
                    self.candle_data.remove(&symbol);
                    ...
                    if let Err(e) = self.ws_manager.write(
                        ...
                        log::error!("Failed to unsubscribe from candle for {}: {}", symbol, e);
                    } else {
                        log::info!("Removed Binance candle subscription for {}", symbol);
                    }
                }
            }
            CandleCommand::GetCandle { symbol, response } => {
                let result = self.candle_data.get(&symbol).cloned();
    ...
    fn handle_message(&mut self, content: &str) {
        if let Ok(candle_msg) = serde_json::from_str::<BinanceCandleMessage>(content) {
            ...
            self.candle_data.insert(symbol, candle);
        }
    }
}
