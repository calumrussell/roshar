use roshar_types::{ByBitCandleMessage, Candle, ByBitWssMessage};
...
impl CandleFeed {
    pub fn new(ws_manager: Arc<Manager>) -> Self {
        ...
        Self {
            ws_manager,
            conn_name: "bybit-candle".to_string(),
    ...
    pub async fn run(mut self) {
        ...
        log::info!(
            "WebSocket reader set up for ByBit candle feed: {}",
            self.conn_name
        );
        ...
        if let Err(e) = self.ws_manager.new_conn(&self.conn_name, ws_config) {
            log::error!(
                "Failed to create ByBit candle WebSocket connection: {}",
                e
            );
            return;
        }
                ...
                msg = recv.recv() => {
                    match msg {
                        Ok(roshar_ws_mgr::Message::SuccessfulHandshake(_name)) => {
                            log::info!("ByBit candle feed WebSocket connected: {}", self.conn_name);
                        ...
                        Ok(roshar_ws_mgr::Message::ReadError(_name, err)) => {
                            log::error!("Websocket read error in ByBit candle feed: {}", err);
                            self.is_connected = false;
                        }
                        Ok(roshar_ws_mgr::Message::WriteError(_name, err)) => {
                            log::error!("Websocket write error in ByBit candle feed: {}", err);
                        }
                        Ok(roshar_ws_mgr::Message::PongReceiveTimeoutError(_name)) => {
                            log::warn!("Pong receive timeout in ByBit candle feed");
                            self.is_connected = false;
                        }
                        Ok(_) => {}
                        Err(e) => {
            ...
            CandleCommand::Add { symbol } => {
                if self.subscriptions.insert(symbol.clone()) {
                    ...
                    if let Err(e) = self.ws_manager.write(
                        ...
                        log::info!("Subscribed to ByBit candle feed for {}", symbol);
                    }
                }
            }
    ...
    fn handle_message(&mut self, content: &str) {
        if let Ok(candle_msg) = serde_json::from_str::<ByBitCandleMessage>(content) {
            let candles = candle_msg.to_candles();
